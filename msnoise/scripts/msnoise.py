...
@click.command()
@click.option('-p', '--pcc', is_flag=True, help='Compute the phase cross-correlation - Schimmel style')
@click.option('-x', '--pxc', is_flag=True, help='Compute the phase cross-correlation - Lion style')
@click.pass_context
def compute_ac(ctx, pcc, pxc):
    """Computes the AC jobs with PCC (based on the "New Jobs" identified)"""
    if pcc:
        from ..s034compute_pcc import main
        click.secho('Using PCC...', fg='green')
    elif pxc:
        from ..s035compute_pcc import main
        click.secho('Using PCC...', fg='green')
    else:
...
@click.pass_context
def interferogram(ctx, sta1, sta2, filterid, comp, mov_stack, show, outfile, maxlag):

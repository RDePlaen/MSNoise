...
def main(stype, interval=1):
    ...
    logging.debug('Starting the %s stack for SC' % stype)
    db = connect()
    ...
    for f in get_filters(db, all=False):
        ...
        for components in pairs:
            for station in stations:
                ...
                updated_days = updated_days_for_dates(db, start, end, station, jobtype='SC', interval=datetime.timedelta(days=interval),returndays=True)
